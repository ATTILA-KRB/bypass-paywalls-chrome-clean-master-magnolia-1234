...
ext_api.webRequest.onBeforeSendHeaders.addListener((details) => {
    ...
    requestHeaders = requestHeaders.map(requestHeader => {
      if (requestHeader.name === 'Referer') {
        ...
        for (const [key, value] of Object.entries(refererMap)) {
          if (key === 'google' && (googlebotEnabled || matchUrlDomain(use_google_referer, details.url))) {
            requestHeader.value = value;
            setReferer = true;
            return requestHeader;
          } else if (key === 'facebook' && matchUrlDomain(use_facebook_referer, details.url)) {
            requestHeader.value = value;
            setReferer = true;
            return requestHeader;
          } else if (key === 'twitter' && matchUrlDomain(use_twitter_referer, details.url)) {
            requestHeader.value = value;
            setReferer = true;
            return requestHeader;
          }
        ...
        const domain = matchUrlDomain(use_referer_custom, details.url);
        if (domain) {
  ...
  if (!matchUrlDomain(allow_cookies, details.url)) {
    requestHeaders = requestHeaders.filter(requestHeader => 
  ...
  if (kiwi_browser) {
    ...
    const queryTabs = () => ext_api.tabs.query({ active: true, currentWindow: true }, tabs => {
      if (tabs.length > 0 && /^http/.test(tabs[0].url)) {
        const tab = tabs[0];
...
async function check_sites_custom_ext() {
  try {
    ...
    customSitesExt = json.map(site => site.domain); // Optimized mapping
    const removeSites = json.find(site => site['###_remove_sites']);
    if (removeSites?.cs_code) {
      customSitesExt_remove = removeSites.cs_code.split(/,\s+/);
    }
  } catch (err) {
...
let customSitesExt = [];
let customSitesExt_remove = [];
const sites_custom_ext_json = 'custom/sites_custom.json';

ext_api.tabs.onUpdated.addListener((tabId, info, tab) => updateBadge(tab));
ext_api.tabs.onActivated.addListener(activeInfo => {
  if (activeInfo.tabId) {
    ext_api.tabs.get(activeInfo.tabId, updateBadge);
  }
});

function updateBadge(activeTab) {
  if (ext_api.runtime.lastError || !activeTab || !activeTab.active) return;
  ...
  if (currentUrl) {
    ...
    const isDefaultSite = matchUrlDomain(defaultSites_domains, currentUrl);
    const isCustomSite = matchUrlDomain(customSites_domains, currentUrl);
    const isUpdatedSite = matchUrlDomain(updatedSites_domains_new, currentUrl);
    ...
    if (!isDefaultSite && (isCustomSite || isUpdatedSite)) {
      ext_api.permissions.contains({
        ...
        updateBadgeTextAndColor(badgeText, color);
      });
    } else {
...
function updateBadgeTextAndColor(badgeText, color) {
  if (color && badgeText) {
...
function setExtVersionNew(check_ext_version_new, check_ext_upd_version_new = '') {
  ext_api.management.getSelf(async function (result) {
    const installType = result.installType;
    const ext_version_len = (installType === 'development') ? 7 : 5;
    let newExtVersion = check_ext_version_new;

    if (ext_version_len === 5 && check_ext_upd_version_new && check_ext_upd_version_new < check_ext_version_new) {
      newExtVersion = check_ext_upd_version_new;
    }

    if (newExtVersion && newExtVersion.substring(0, ext_version_len) <= ext_version.substring(0, ext_version_len)) {
      newExtVersion = '1';
    }

    await ext_api.storage.local.set({ ext_version_new: newExtVersion });
  });
}
...
async function check_update() {
  ...
  try {
    ...
    if (!response.ok) throw new Error('Erreur lors de la récupération du manifeste');
    
    const json = await response.json();
    const json_ext_version_new = json['version'];
    
    const updateUrl = manifestData.browser_specific_settings?.gecko?.update_url;
    if (updateUrl) {
      try {
        const response = await fetch(updateUrl);
        if (!response.ok) throw new Error('Erreur lors de la récupération de la mise à jour');
...
function site_switch() {
  ext_api.tabs.query({ active: true, currentWindow: true }, async function (tabs) {
    if (tabs.length > 0 && tabs[0].url.startsWith('http')) {
      const currentUrl = tabs[0].url;
      let isDefaultSite = matchUrlDomain(defaultSites_grouped_domains, currentUrl) || 
                         Object.keys(grouped_sites).some(key => grouped_sites[key].includes(matchUrlDomain(defaultSites_domains, currentUrl)));
      ...
      if (!isDefaultSite) {
        ...
        const isUpdatedSite = matchUrlDomain(sites_updated_domains_new, currentUrl) || 
                              Object.values(updatedSites)
                                ...
                                .some(group => matchUrlDomain([group], currentUrl));
      ...
      if (domain && site_title) {
        ...
        const items = await ext_api.storage.local.get({ sites: {} });
        const sites = items.sites || {};
        ...
        await ext_api.storage.local.set({ sites });
        ext_api.tabs.reload({ bypassCache: true });
      }
    }
  });
...
function remove_cookies_fn(domainVar, exclusions = false) {
  ext_api.cookies.getAllCookieStores(cookieStores => {
    ext_api.tabs.query({ active: true, currentWindow: true }, tabs => {
      if (!ext_api.runtime.lastError && tabs.length > 0 && tabs[0].url.startsWith('http')) {
        ...
        const cookie_get_options = { domain: domainVar, ...(storeId !== '0' && { storeId }) };
        ext_api.cookies.getAll(cookie_get_options, cookies => {
          cookies.forEach(cookie => {
            if (exclusions) {
              const rc_domain = cookie.domain.replace(/^(\.?www\.|\.)/, '');
              if ((remove_cookies_select_hold[rc_domain]?.includes(cookie.name) || 
                  (remove_cookies_select_drop[rc_domain] && !remove_cookies_select_drop[rc_domain].includes(cookie.name)) || 
            ...
            const cookie_remove_options = {
              url: `${cookie.secure ? "https://" : "http://"}${cleanDomain}${cookie.path}`,
              name: cookie.name,
              ...(storeId !== '0' && { storeId })
            };
  ...
  ext_api.tabs.query({ active: true, currentWindow: true }, function (tabs) {
    if (tabs.length > 0 && /^http/.test(tabs[0].url)) {
      ext_api.tabs.executeScript({
        ...
        if (ext_api.runtime.lastError || (res && res[0])) {
          console.error('Erreur lors de l\'exécution du script :', ext_api.runtime.lastError);
          return;
        }
      });
      ext_api.tabs.update(tabs[0].id, { url: tabs[0].url });
    }
  });
}
...
ext_api.runtime.onMessage.addListener((message, sender) => {
  switch (message.request) {
    ...
    async function getArticleSrc(message) {
      const url_src = message.data.url_src || message.data.url;
      try {
        ...
        if (message.data.url.startsWith('https://archive.') && url_src.includes('/https')) {
          const textBlockIndex = html.indexOf('<div class="TEXT-BLOCK"');
          if (textBlockIndex !== -1) {
            message.data.url_src = html.slice(textBlockIndex).split('</div>')[0].split('href="')[1].split('"')[0];
            await getArticleSrc(message);
            recursive = true;
          } else {
            html = '';
          }
        ...
        if (!recursive) {
          if (html) {
            ...
            if (typeof DOMParser === 'function') {
              ...
              const article_new = doc.querySelector(message.data.selector_source);
              html = article_new ? article_new.outerHTML : '';
            }
          }
  ...
  if (message.scheme && (![chrome_scheme, 'undefined'].includes(message.scheme) || focus_changed)) {
    const icon_path = { path: { '128': message.scheme === 'dark' ? 'bypass-dark.png' : 'bypass.png' } };
...
ext_api.storage.local.get(["optInShown", "customShown"]).then(result => {
  const optInNeeded = !result.optInShown || !result.customShown;
  if (optInNeeded) {
...
function filterObject(obj, filterFn, mapFn = (val, key) => [key, val]) {
  return Object.fromEntries(
    Object.entries(obj)
      .filter(([key, val]) => filterFn(val, key))
      .map(([key, val]) => mapFn(val, key))
  );
}
function compareKey(firstStr, secondStr) {
  const normalize = str => str.toLowerCase().trim().replace(/\s\(.*\)/, '');
  return normalize(firstStr) === normalize(secondStr);
}
...
function isSiteEnabled(details) {
  ...
  return enabledSite in restrictions ? restrictions[enabledSite].test(details.url) : !!enabledSite;
}
...
function randomIP(range_low = 0, range_high = 223) {
  ...
  for (let n = 1; n < 4; n++) {
    rndmIP.push(randomInt(256)); // 0-255 inclusif
  }
  return rndmIP.join('.');
}

// Rafraîchir l'onglet actuel (http)
function refreshCurrentTab() {
  ext_api.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    if (tabs?.[0]?.url && /^http/.test(tabs[0].url)) {
      ext_api.tabs.update(tabs[0].id, { url: tabs[0].url });
    }
  });
}
